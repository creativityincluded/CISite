...
class Cart66AccessManager {
  ...
  public static function getPrivatePageIds() {
    $privatePagesAndPosts = array();
    $pages = get_pages(array('meta_key'=>'cart66_access', 'meta_value' => 'private', 'hierarchical' => 0, 'numberposts' => -1));
    $posts = get_posts(array('meta_key'=>'cart66_access', 'meta_value' => 'private', 'hierarchical' => 0, 'numberposts' => -1));
    $pagesAndPosts = array_merge($pages, $posts);
    foreach($pagesAndPosts as $pnp) {
      $privatePagesAndPosts[] = $pnp->ID;
    }
    return $privatePagesAndPosts;
  }
